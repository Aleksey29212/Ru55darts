...
import { addDoc, collection, doc, serverTimestamp, updateDoc, deleteField, deleteDoc, getDoc, getDocs, writeBatch } from 'firebase/firestore';
...
export async function importTournament(prevState: unknown, formData: FormData) {
  ...
  for (const input of inputs) {
    try {
      ...
      // ГАРАНТИЯ: Проверка на дубликаты перед добавлением
...
export async function updatePlayerAvatar(playerId: string, dataUrl: string | null) {
    try {
        ...
        if (dataUrl) {
            ...
            const playerSnap = await getPlayerProfileById(playerId);
            const name = playerSnap?.name || playerId;
...
async function getPlayerProfileById(id: string) {
    const db = getDb();
