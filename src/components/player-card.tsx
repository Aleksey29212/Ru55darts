                    ...
                    <div>
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-3 md:gap-5 lg:gap-6">
                            <StatItem 
                                ...
                                value={player.points} 
                                caption="ИТГ"
                                description="Суммарный рейтинг игрока в этой лиге. Складывается из базовых очков за занятые места (ТОП-16) и всех заработанных бонусов за сезон. Является главным показателем в иерархии рейтинга." 
                            />
                            <StatItem 
                                template={template} 
                                label="ТУР." 
                                ...
                                caption="МТЧ"
                                description="Общий стаж игрока в системе DartBrig Pro. Показывает количество официальных турниров, в которых спортсмен принял участие и зафиксировал результат в протоколе." 
                            />
                            <StatItem 
                                template={template} 
                                label="Т-8" 
                                ...
                                value={player.wins} 
                                caption="П-ОФ"
                                description="Показатель мастерства прохождения сетки. Учитывает все выходы в 1/4 финала и выше. Высокий показатель говорит о хладнокровии в решающих стадиях туров." 
                            />
                            <StatItem 
                                template={template} 
                                label="ГР." 
                                ...
                                value={player.losses} 
                                caption="ВЫЛ"
                                description="Количество турниров, в которых игрок завершил выступление на групповой стадии. Помогает оценить стабильность прохождения квалификационных этапов." 
                            />
                            <StatItem 
                                template={template} 
                                label="ЭФФ." 
                                ...
                                value={winRate} 
                                caption="ПЦТ"
                                description="Коэффициент результативности (Efficiency). Показывает процент выходов в стадию плей-офф (ТОП-8) относительно общего количества сыгранных турниров." 
                            />
                        </div>
                    </div>
                        ...
                        <div className={cn(
                            ...
                            <div className="grid grid-cols-2 gap-4 sm:gap-6 relative z-10 mt-auto">
                                <StatItem 
                                    ...
                                    caption="ЗА МЕСТА"
                                    description="Очки за чисто спортивный результат. Начисляются согласно официальной таблице рангов лиги (только для ТОП-16) + баллы за участие, если они предусмотрены лигой." 
                            />
                                <StatItem 
                                    template={template} 
                                    label="БОНУСЫ" 
                                    ...
                                    caption="БОНУСЫ"
                                    description="Баллы за профессиональные достижения: заходы в 180, высокие чекауты (Hi-Out), короткие леги и высокий AVG. Эти баллы помогают подниматься в рейтинге при равенстве мест." 
                                />
                            </div>
                        </div>
                        ...
                        <div className={cn(
                            ...
                            <div className="grid grid-cols-3 gap-2 sm:gap-3 relative z-10 mt-auto">
                                <StatItem 
                                    ...
                                    caption="НАБОР"
                                    description="Average (Средний набор). Ключевой индикатор класса дартсмена: среднее количество очков за один подход. Применяется как первый критерий при равенстве баллов в рейтинге." 
                                />
                                <StatItem 
                                    template={template} 
                                    label="180" 
                                    ...
                                    value={player.n180s} 
                                    caption="МАКСИМУМЫ"
                                    description="Общее количество 'максимумов' (попадание всеми тремя дротиками в сектор T20). Показатель взрывной мощи игрока и точности его кучности." 
                                />
                                <StatItem 
                                    template={template} 
                                    label="Hi-Out" 
                                    ...
                                    caption="ФИНИШ"
                                    description="Max Out (Максимальный чекаут). Лучший результат закрытия лега одним подходом. Демонстрирует хладнокровие на финише и является вторым критерием тай-брейка в таблице." 
                                />
                            </div>
                        </div>
                    </div>
