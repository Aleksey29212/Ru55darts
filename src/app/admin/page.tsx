...
import { ArrowRight, Calculator, Users, Wand2, Trophy, Camera, Library, Handshake, Image, BarChart, CloudDownload, Code2, AlertOctagon } from "lucide-react";
...
function StatCard({ title, count, icon: Icon, isLoading, priority = false }: { title: string, count: number, icon: any, isLoading: boolean, priority?: boolean }) {
    return (
        <Card className={cn(
            "glassmorphism border-primary/20 transition-all duration-150",
            ...
            <section className="grid md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                {adminSections.map(section => (
                    <Button key={section.href} asChild variant="outline" className={cn(
                        "h-auto py-8 justify-start glassmorphism border-white/5 hover:border-primary/50 group transition-all duration-150 rounded-2xl interactive-scale",
